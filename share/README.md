...
- **토큰** (provider 가 보고한 usage 기준; 자체 추정 금지)
  - `inputTokens`(**cache read 제외**) / `outputTokens`(reasoning 포함) /
    `cacheReadTokens` / `cacheCreationTokens`(없으면 0/생략) / `requests`(provider API 호출 수)
  - provider(Claude/OpenAI/vLLM)별 변환은 `docs/IMPLEMENTATION_NOTES.md` 참고.
- **확정 / 응답 규칙**
